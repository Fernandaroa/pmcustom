  ...
  return (
    <form onSubmit={onSubmit} className="grid gap-5">
      <div className="grid md:grid-cols-2 gap-5">
        <input className="p2-input" placeholder={t("fName")} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} maxLength={100} required />
        <input type="email" className="p2-input" placeholder={t("fEmail")} value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} maxLength={150} required />
      </div>
      <input className="p2-input" placeholder={t("fCompany")} value={form.company} onChange={(e) => setForm({ ...form, company: e.target.value })} maxLength={120} />
      <textarea className="p2-input resize-none" rows={5} placeholder={t("fMessage")} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} maxLength={1000} required />
      <div>
